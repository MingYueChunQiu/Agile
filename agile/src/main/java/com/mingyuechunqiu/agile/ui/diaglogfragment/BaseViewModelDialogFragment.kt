 ...
 * </pre>
 */
abstract class BaseViewModelDialogFragment : BaseDialogFragment(), IViewModelOwner {
    ...
    private fun registerAgileResourceObserver(
        ...
        viewModel.apply {
            ...
            getPopHintState().observe(lifecycleOwner) {
                when (it) {
                    is IBaseViewModel.PopHintState.MsgResIdToast -> this@BaseViewModelDialogFragment.showToast(
                        it.msgResId
                    )
                    is IBaseViewModel.PopHintState.MsgToast -> this@BaseViewModelDialogFragment.showToast(
                        it.msg
                    )
                    is IBaseViewModel.PopHintState.ErrorInfoToast -> this@BaseViewModelDialogFragment.showToast(
                        it.info
                    )
                    is IBaseViewModel.PopHintState.ConfigToast -> this@BaseViewModelDialogFragment.showToast(
                        it.config
                    )
                }
            }
            getStatusViewState().observe(lifecycleOwner) {
                when (it) {
                    is IBaseViewModel.StatusViewState.ShowContainerIdLoading -> this@BaseViewModelDialogFragment.showLoadingStatusView(
                        it.containerId
                    )
                    is IBaseViewModel.StatusViewState.ShowHintLoading -> this@BaseViewModelDialogFragment.showLoadingStatusView(
                        it.hint,
                        it.cancelable
                    )
                    is IBaseViewModel.StatusViewState.Dismiss -> this@BaseViewModelDialogFragment.dismissStatusView()
                }
            }
        }
    }
